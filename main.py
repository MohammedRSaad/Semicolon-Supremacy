...
class classrooms:
    ...
    def take_attendence(self, name=None, Id=None):
        ...
        self.students[f"{date}"][self.students['id'] == Id] = 0
...
class schools:
    ...
    def transfer_student(self, name, to_class, from_class):
        to_class.students.loc[self.students.shape[0]+1] = to_class.students[self.students["name"] == name]
...
class App(tk.Tk):
    ...
    def main_win(self, school):
        ...
        new_classroom = ttk.Button(self.main, text='New Classroom', command=lambda:\
                      [self.new_classroom_win(school), self.main.pack_forget()])
        new_classroom.grid(column=0, row=0, sticky=tk.E, padx=5, pady=5)
        
        new_student = ttk.Button(self.main, text='Add Student', command=lambda:\
                      [self.add_student_win(school), self.main.pack_forget()])
        new_student.grid(column=0, row=1, sticky=tk.E, padx=5, pady=5)
        
        transefer_frame = ttk.Button(self.main, text='Transfer Student', command=lambda:\
                      [self.transfer_win(school), self.main.pack_forget()])
        transefer_frame.grid(column=0, row=2, sticky=tk.E, padx=5, pady=5)
        
        
        take_attendance = ttk.Button(self.main, text='Take Attendance', command=lambda:\
                      [self.take_attendance_win(school), self.main.pack_forget()])
        take_attendance.grid(column=0, row=3, sticky=tk.E, padx=5, pady=5)
        
        show = ttk.Button(self.main, text='show Classrooms', command=lambda:\
    ...
    def take_attendance_win(self, school):
        ...
        submit = ttk.Button(self.take_attendance, text='Submit', command=lambda:\
                      [school.classes[school.names.index(var.get())].take_attendance(var.get())])
